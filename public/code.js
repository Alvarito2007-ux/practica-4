...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Cargar artí­culos existentes. Recibe dos parámetros, la página a 
    // mostrar y los artículos por página. Por defecto si no se indica 
    // nada muestra la primera página con dos artículos por página
    const loadArticles = async (page = 1, limit = limitArticlesPerPage) => {
        try {
     // Obtenemos por GET la lista de artículos
 ...
 // Recorremos el array de articles 
            ...
            console.error('Error al cargar artí­culos:', error);
            showError('Error al cargar artí­culos');
        }
    };
    ...
    try {
     // Usamos el método POST para subir el fichero al server
        ...
        if (response.ok) {
            ...
            showError('Error al subir la imagen 1');
        }
    } catch (error) {
        console.error('Error al subir imagen:', error);
        showError('Error al subir la imagen 2');
    }
});
